    ...
    def getGrafoTransposto(self):
        ...
        for vertice in self.vertices.keys():
            novoDicVertices.update(
                    {vertice: {
                        "rotulo": self.vertices.get(vertice).get('rotulo'),"indexDasArestas": []}})

        for vertice, vizinho, peso in self.arestas:
            size = len(novaListaArestas)
            indexesVizinho = novoDicVertices.get(vizinho).get("indexDasArestas")
            indexesVizinho.append(size)
            novoDicVertices.get(vizinho).update({vertice: peso,"indexDasArestas": indexesVizinho})
            novaListaArestas.append((vizinho, vertice, peso))
        return GrafoDirigido2(vertices=novoDicVertices, arestas=novaListaArestas)